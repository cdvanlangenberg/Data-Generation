...
# 
# nl =2 ; nL = 76; N = nL/2;  m = 0:(2*N);
# C1=1 ; C2= 1 ; a=1 ; u=1; p=.5
# 
# # phi <- round(seq(0, pi, by=pi/(nl+1))[-c(1,nl+2)],1)
# phi <- c(10,15)*pi/180
...
for(hh in 1:nn){
  ...
  var_sim[hh,jj]<-(1/nL)*(sum(Y[1,]*Y[2,]))-mean(Y[1,])*mean(Y[2,]) #+mycov(phi[1],phi[2])
  #print(mean(Y[1,]))
    #cov(Y[1,],Y[2,])
...
var_phi <- mycov(phi[1],phi[2])*(1-p^2)/(1-2*p*cos(theta)+p^2) #-mycov(phi[1],phi[2])

#var_phi <- mycov(phi[1],phi[2])*(1-p^2)/(1-2*p*cos(lambda+u*(phi[1]-phi[2]))+p^2)
# C1*(C2-exp(-a*abs(phi[1]))-exp(-a*abs(phi[2]))+exp(-a*abs(phi[1]-phi[2])))
# C1*(C2-(1/sqrt(a^2+phi[1]^2))-(1/sqrt(a^2+phi[2]^2))+(1/sqrt(a^2+(phi[1]-phi[2])^2)))

plot(lambda, var_phi, type="l", lty=2, col=3, ylim=c(min(var_sim), max(var_phi)), xaxt = "n", bty="n",ylab="Covariance",
     main=paste("nl=",nl,"nL=",nL, "p=",p, "phi=", phi[1]*180/pi,"&", phi[2]*180/pi), sub=paste("#simulations=",nn))
axis(1, at=lambda, labels=round(lambda*180/pi,0))
lines(lambda, var_sim, type="l", lty=3, col=4, lwd=2)
legend("top", c("formula","simulation"), lty=c(2,3), col=3:4, bty="n", lwd=1:2)
