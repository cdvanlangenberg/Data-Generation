...
# 
# nl =2 ; nL = 16; N = nL/2;  m = 0:N;
# C1=1 ; C2= 1 ; a=1 ; u=1; p=.7
# phi <- c(10,15)*pi/180
...
v <- C1*(C2-(1/sqrt(a^2+phi[1]^2))-(1/sqrt(a^2+phi[2]^2))+(1/sqrt(a^2+(phi[1]-phi[2])^2)))

v0 <- v*(1-p^2)/(1-2*p*cos(u*(phi[1]-phi[2]))+p^2)
...
plot(lambda[lambda>=0], var_sim[lambda>=0], type="o", lty=2, col=3, xaxt = "n", bty="n",ylab="Variance",ylim=c(min(var_sim),max(var_sim,var_phi)),pch=19,
