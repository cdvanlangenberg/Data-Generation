...
# calculate empirical cross-variograms and then compare them with theoretical variograms
# This is the best version of random seed numbers to produce the plots
...
#For model 3 - not working for u = 0 ==================================
#v <- mycov(phi[1], phi[2])
#v0 <- 1 * v * (1 - cos(u * (phi[1] - phi[2])))))
#v1 <- 1 * v * log(1 / (2 * (1 - cos(l1 + u * (phi[1] - phi[2])))))
#v2 <- 1 * v * log(1 / (2 * (1 - cos(l2 + u * (phi[1] - phi[2])))))
#var_phi <- 1*(v0 - .5 * (v1 + v2))
#=====================================================================
...
#store the results from both simulation and theory (10, 80), (20, 80), ... (70, 80), (80, 80)
# var_sim_all <- list()
# var_sim_all[[1]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[2]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[3]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[4]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[5]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[6]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[7]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[8]] <- cbind(var_sim[lambda %in% l1], var_phi)
# var_sim_all[[9]] <- cbind(var_sim[lambda %in% l1], var_phi)
